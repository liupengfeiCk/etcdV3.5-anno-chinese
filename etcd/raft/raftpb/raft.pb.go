...
const (
	MsgHup            MessageType = 0  //发起选举消息
	MsgBeat           MessageType = 1  //本地心跳消息
	MsgProp           MessageType = 2  //客户端写请求
	MsgApp            MessageType = 3  //app消息
	MsgAppResp        MessageType = 4  //app消息返回消息
	MsgVote           MessageType = 5  //选举消息
	MsgVoteResp       MessageType = 6  //选举消息返回消息
	MsgSnap           MessageType = 7  //快照复制消息
	MsgHeartbeat      MessageType = 8  //心跳消息
	MsgHeartbeatResp  MessageType = 9  //心跳消息返回消息
	MsgUnreachable    MessageType = 10 //leader发送MsgSnap消息失败后发送，表示当前节点无法与指定节点通讯
	MsgSnapStatus     MessageType = 11 //leader发送MsgSnap消息失败后发送
	MsgCheckQuorum    MessageType = 12 //领导者自检消息，本地消息
	MsgTransferLeader MessageType = 13 //节点转移消息，本地消息
	MsgTimeoutNow     MessageType = 14 //通知follower马上强制进行领导选举
	MsgReadIndex      MessageType = 15 //只读请求，即客户端发来的读取请求
	MsgReadIndexResp  MessageType = 16 //只读请求的响应
	MsgPreVote        MessageType = 17 //预选举消息
	MsgPreVoteResp    MessageType = 18 //预选举消息返回消息
)
...
type SnapshotMetadata struct { //快照元数据
	// conf状态
...
type HardState struct {
	...
	Vote   uint64 `protobuf:"varint,2,opt,name=vote" json:"vote"`     //投票给哪个节点
	Commit uint64 `protobuf:"varint,3,opt,name=commit" json:"commit"` //最后一条已提交记录的索引值
}
