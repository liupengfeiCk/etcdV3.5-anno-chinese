...
func toConfChangeSingle(cs pb.ConfState) (out []pb.ConfChangeSingle, in []pb.ConfChangeSingle) {
	...
	for _, id := range cs.VotersOutgoing { //获取离开voter的节点作为add节点
	...
	for _, id := range cs.VotersOutgoing { //获取离开voter的节点作为remove的节点
	...
	for _, id := range cs.Voters { //然后获取要加入的节点作为add节点
		in = append(in, pb.ConfChangeSingle{
			Type:   pb.ConfChangeAddNode,
			NodeID: id,
		})
	...
	for _, id := range cs.Learners {
		in = append(in, pb.ConfChangeSingle{ //获取学习者作为addLearner节点
	...
	for _, id := range cs.LearnersNext { //获取要成为学习者的节点作为addLearner节点
		in = append(in, pb.ConfChangeSingle{
			Type:   pb.ConfChangeAddLearnerNode,
			NodeID: id,
		})
...
func Restore(chg Changer, cs pb.ConfState) (tracker.Config, tracker.ProgressMap, error) {
	outgoing, incoming := toConfChangeSingle(cs) //获取出去和进来的节点

	var ops []func(Changer) (tracker.Config, tracker.ProgressMap, error)

	if len(outgoing) == 0 { //如果没有出去的节点
		...
		// into a joint state.
		ops = append(ops, func(chg Changer) (tracker.Config, tracker.ProgressMap, error) { //然后执行in的更改
			return chg.EnterJoint(cs.AutoLeave, incoming...) //联合配置的操作
		})
	}
