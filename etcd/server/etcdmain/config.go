...
func (cfg *config) parse(arguments []string) error {
	perr := cfg.cf.flagSet.Parse(arguments) //解析参数并检查参数是否有-help或-h，有则执行帮助
	switch perr {
	case nil:
	case flag.ErrHelp:
	...
	if len(cfg.cf.flagSet.Args()) != 0 { //返回非定义的参数
		return fmt.Errorf("'%s' is not a valid flag", cfg.cf.flagSet.Arg(0))
	}

	if cfg.printVersion { //如果使用了-version 则输出版本
	...
	if cfg.configFile == "" { //判断是否使用配置文件而弃用命令行参数
		cfg.configFile = os.Getenv(flags.FlagToEnv("ETCD", "config-file"))
	}

	if cfg.configFile != "" { //如果文件不为空则启用配置文件参数忽略命令行
...
func (cfg *config) configFromCmdLine() error {
	...
	lg, err := zap.NewProduction() //指定日志输出
	if err != nil {
		return err
	}
	...
	err = flags.SetFlagsFromEnv(lg, "ETCD", cfg.cf.flagSet) //解析参数
	if err != nil {
		return err
	}

	if rafthttp.ConnReadTimeout < rafthttp.DefaultConnReadTimeout { //raft读取超时时间最小值不能小于5秒
	...
	if rafthttp.ConnWriteTimeout < rafthttp.DefaultConnWriteTimeout { //raft写入超时时间不能小于5秒
