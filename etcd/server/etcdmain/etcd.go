...
func startEtcdOrProxyV2(args []string) {
	...
	cfg := newConfig() //创建一个新的etcd配置
	defaultInitialCluster := cfg.ec.InitialCluster

	err := cfg.parse(args[1:]) //解析参数，从配置文件中或者从命令行
	lg := cfg.ec.GetLogger()   //获取日志对象
	...
	defaultHost, dhErr := (&cfg.ec).UpdateDefaultClusterFromName(defaultInitialCluster) //如果设置了默认广播主机，则取出默认主机的host
	if defaultHost != "" {
		lg.Info(
	...
	if cfg.ec.Dir == "" { //如果未设置文件夹则设置默认文件夹名
	...
	which := identifyDataDirOrDie(cfg.ec.GetLogger(), cfg.ec.Dir) //获取数据文件夹类型
	if which != dirEmpty {                                        //如果文件类型不为空文件，则直接启动etcd服务
		lg.Info(
	...
	} else { //否则认为是一个新服务，直接启动etcd并验证集群是否已满
		shouldProxy := cfg.isProxy()
		if !shouldProxy {
			stopped, errc, err = startEtcd(&cfg.ec)
			if derr, ok := err.(*etcdserver.DiscoveryError); ok && derr.Err == v2discovery.ErrFullCluster { //如果集群已满则返回代理
				if cfg.shouldFallbackToProxy() {
					...
					shouldProxy = true //将代理设为true
				}
			} else if err != nil { //如果是其他错误则记录日志
				lg.Warn("failed to start etcd", zap.Error(err))
			}
		}
		if shouldProxy { //如果代理被设为true则启动代理
			err = startProxy(cfg)
		}
	}
	...
	if err != nil { //如果启动发生错误
		if derr, ok := err.(*etcdserver.DiscoveryError); ok {
			switch derr.Err {
			case v2discovery.ErrDuplicateID: //找到重复的id
				lg.Warn(
			...
			case v2discovery.ErrDuplicateName: //找到重复的名称
				lg.Warn(
		...
		if strings.Contains(err.Error(), "include") && strings.Contains(err.Error(), "--initial-cluster") { //如果错误包含--initial-cluster
	...
	osutil.HandleInterrupts(lg) //调用一个线程来接收 SIGINT or SIGTERM 信号并结束etcd进程
	...
	// connections.
	notifySystemd(lg) //通知服务管理器，服务已启动完成
...
func startEtcd(cfg *embed.Config) (<-chan struct{}, <-chan error, error) {
	...
	osutil.RegisterInterruptHandler(e.Close) //增加中断处理程序
	select {
	case <-e.Server.ReadyNotify(): // wait for e.Server to join the cluster //等待server加入集群
	case <-e.Server.StopNotify(): // publish aborted from 'ErrStopped' //不懂？
	}
