...
func StartEtcd(inCfg *Config) (e *Etcd, err error) {
	if err = inCfg.Validate(); err != nil { //验证cfg
		return nil, err
	}
	serving := false
	e = &Etcd{cfg: *inCfg, stopc: make(chan struct{})} //新建一个etcd实例
	cfg := &e.cfg
	defer func() { //在最后关闭etcd
		if e == nil || err == nil { //如果没有错误或etcd为空则不执行
			return
		}
		if !serving {
	...
	if !cfg.SocketOpts.Empty() { //如果socketopts不为空 socketopts有啥用？
		cfg.logger.Info(
	...
	if e.Peers, err = configurePeerListeners(cfg); err != nil { //配置对等监听者
		return e, err
	}
	...
	if e.sctxs, err = configureClientListeners(cfg); err != nil { //配置客户端监听
		return e, err
	}

	for _, sctx := range e.sctxs { //将客户端监听放入etcd的clients集合中
	...
	if !isMemberInitialized(cfg) { //如果该成员的member/war文件夹内没有任何文件，则认为其没有初始化
		memberInitialized = false
		urlsmap, token, err = cfg.PeerURLsMapAndToken("etcd") //获取其初始化的token和对等服务器的url
		if err != nil {
	...
	//解析自动压缩配置，默认为按时间压缩，且时间为0，即不压缩
	autoCompactionRetention, err := parseCompactionRetention(cfg.AutoCompactionMode, cfg.AutoCompactionRetention)
	if err != nil {
		return e, err
	}

	backendFreelistType := parseBackendFreelistType(cfg.BackendFreelistType) //指定后端DB所使用的数据结构类型，默认为hashmap

	srvcfg := config.ServerConfig{ //读取配置到etcd服务器
		Name:                                     cfg.Name,
	...
	if srvcfg.ExperimentalEnableDistributedTracing { //开启分布式跟踪，具体做啥不知道
	...
	print(e.cfg.logger, *cfg, srvcfg, memberInitialized) //打印配置信息

	if e.Server, err = etcdserver.NewServer(srvcfg); err != nil { //创建服务器实例
		return e, err
	}

	// buffer channel so goroutines on closed connections won't wait forever
	e.errc = make(chan error, len(e.Peers)+len(e.Clients)+2*len(e.sctxs)) //不知道干嘛

	// newly started member ("memberInitialized==false")
	// does not need corruption check
	if memberInitialized { //如果该服务器的成员目录已经初始化过，说明是重启，需要检查hashKV
	...
	if err = e.serveMetrics(); err != nil { //启动指标监控
		return e, err
	}
