...
type store struct {
	Root           *node           //节点，存储数据的结构
	WatcherHub     *watcherHub     //观察者集合
	CurrentIndex   uint64          //当前index
	Stats          *Stats          //状态，不知道有啥
	CurrentVersion int             //当前版本
	ttlKeyHeap     *ttlKeyHeap     // need to recovery manually //暂时key的堆，是小根堆
	worldLock      sync.RWMutex    // stop the world lock //the world 懂得都懂
	clock          clockwork.Clock //一个准确的时钟
	readonlySet    types.Set       //只读集合
}
